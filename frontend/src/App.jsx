...
import { Sparkles, Heart, Loader2, RefreshCw, AlertCircle, Search as SearchIcon, Ghost } from 'lucide-react';
...
function App() {
  ...
  const isMinimized = (hasResults || (hasSearched && !isLoading)) && !isRefining;
    ...
    <div className={`h-screen w-screen flex flex-col bg-[#050505] text-white selection:bg-violet-500/30 ${isArabic ? 'rtl font-arabic' : 'ltr font-sans'}`} dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      <main className="flex-grow flex flex-col min-h-0 relative px-6 overflow-hidden">
        ...
        <div 
          ...
          } ${hasSearched && !isMinimized ? 'top-10' : ''}`}
          onClick={() => isMinimized && setIsRefining(true)}
        >
          {!hasSearched && !isLoading && !isRefining && (
          ...
          {!hasSearched && !isLoading && !isRefining && (
            <div className="flex flex-wrap justify-center gap-2 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-300">
              {[
                "11 month old can't sleep", 
                "Baby has eating problems", 
                "Safe toys for teething", 
        ...
        <div className={`flex-grow overflow-hidden flex flex-col transition-all duration-700 ${hasResults || isLoading || (hasSearched && !isLoading) ? 'mt-32' : 'mt-0'}`}>
          {isLoading && (
          ...
          <div className="flex-grow overflow-y-auto pb-20 custom-scrollbar">
            <div className="max-w-7xl mx-auto">
              {hasResults ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 animate-in fade-in slide-in-from-top-4 duration-700">
                  <div className="relative">
                    <div className="absolute inset-0 bg-violet-600/20 rounded-full blur-3xl animate-pulse"></div>
                    <Ghost className="w-24 h-24 text-slate-700 relative" />
                  </div>
                  <h3 className="text-2xl font-outfit font-black mt-8 text-white">No Results Found</h3>
                  <p className="text-slate-500 mt-2 text-center max-w-sm">
                    We couldn't find any products matching your query. <br/> Try searching for something else like "Baby Sleep" or "Feeding".
                  </p>
                  <button 
                    onClick={() => setIsRefining(true)}
                    className="mt-8 px-6 py-2 bg-violet-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-violet-500 transition-all active:scale-95"
                  >
                    Refine Search
                  </button>
                </div>
              ...
              {!isLoading && !hasSearched && !error && (
                <div className="h-full flex flex-col items-center justify-center py-20 opacity-20 group">
                  <div className="text-[140px] transition-transform duration-700 group-hover:scale-110">🧸</div>
                  <div className="text-xs font-black uppercase tracking-[0.3em] mt-4">Waiting for Intent</div>
                </div>
              )}
            </div>
          </div>
